...
function devdesk_setup() {
	/*
		...
		* If you're building a theme based on DevDesk, use a find and replace
		* to change 'devdesk' to the name of your theme in all the template files.
		*/
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'devdesk' ),
		)
	);
 ...
 */
function devdesk_scripts() {
	wp_enqueue_style( 'devdesk-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'devdesk-style', 'rtl', 'replace' );

	wp_enqueue_script( 'devdesk-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'devdesk_scripts' );
